...
public class TvSerieEpisodeView extends WebPanel implements TvSerieImageLoaderListener, MouseListener {
	...
	protected void fill(TvSerieEpisode tvSerieEpisode, TvSerieSeason tvSerieSeason) {
		// Title
		...
		this.episodeNumber.setText(String.format("(%d/%d)", tvSerieEpisode.getNumber(), tvSerieSeason.getEpisodeCount()));
		...
		// Directors
		this.directors.fill(TvSerieUtils.getEpisodeDirectors(tvSerieEpisode));
		
		// Writers
		this.writers.fill(TvSerieUtils.getEpisodeWriters(tvSerieEpisode));
		
		// Guest Stars
		this.guestStars.fill(TvSerieUtils.getEpisodeGuestStars(tvSerieEpisode));
	}
	...
	protected Dimension getDirectorSize() {
		return this.directors.getImageSize();
	}
	
	protected Dimension getWriterSize() {
		return this.writers.getImageSize();
	}
	
	protected Dimension getGuestStarSize() {
		return this.guestStars.getImageSize();
	}
	
	protected void destroy() {
		System.out.println("*** TvSerieEpisodeView -> destroy ***");
	...
	private void makeTitlePane() {
		...
		this.episodeNumber = UIUtils.makeStandardLabel("", 18, null);
	...
	private void makeInfoPane() {
		this.firstAired = UIUtils.makeStandardLabel("", 12, null);
	...
	private static class CellRenderer implements ListCellRenderer<FileData> {
		...
		public Component getListCellRendererComponent(JList<? extends FileData> list, FileData value, int index, boolean isSelected, boolean cellHasFocus) {
			WebLabel result = UIUtils.makeStandardLabel(value.getFile(), 12, new Insets(5, 5, 5, 5));
